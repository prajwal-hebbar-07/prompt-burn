...
sources across three operational categories:

1. **Local transcript collectors (disk JSONL):**
   ...
   - **Shared incremental sync** (`src/sync.ts`): drives both transcript trees via SQLite
...
**Double counting in one line:** OMP and Claude Code write separate local transcripts even when
billed against the same Anthropic subscription. Their events are tracked independently and never
deduped against each other; account-level limit cards display the shared provider pool.
...
| `src/sync.ts`            | Module | Shared incremental sync engine for transcript sources     |
...
| `src/sync.test.ts`       | Tests  | Both transcript syncs against temp database with schema   |
...
flowchart TD
    ...
    subgraph Sync ["Synchronous SQLite Transcript Syncs"]
    ...
    C1 & C2 & C3 & S1 & S2 --> RES[Assemble CollectResult]
```
...
2. **Synchronous Transcript Syncs:** `syncOmpSessions` and `syncClaudeSessions` run sequentially,
   each in its own `BEGIN`/`COMMIT` transaction. If disabled, a source returns clean zeros without
   reading disk. If an individual sync throws, it rolls back and returns
   `{ ok: false, error, sync: zeros }`, leaving other sources untouched.
3. **Result Assembly:** All network promises are awaited and returned in `CollectResult`:
   ```ts
   { omp, claudeCode, cursor, ollama, antigravity }
   ```
   Nothing in `collectAllSources` throws; partial success is the normal operating state.

### Transcript Collection (`sync.ts`, `omp.ts`, `claude-code.ts`)

Both transcript syncs run through `syncTranscripts(db, directory, source, scanFile)`:
...
- **Model canonicalization:** Trailing `-YYYYMMDD` date suffixes are stripped by `canonicalModelId`
  (`claude-sonnet-4-5-20250929` → `claude-sonnet-4-5`), while `rawModel` preserves the dated string.
...
- **Shared state table:** `omp_sync_state` (`path`, `mtime`, `offset`) tracks both OMP and Claude
  Code files, keyed by absolute path.
- **Skip check:** If a file's `mtime` matches `Math.floor(stats.mtimeMs)` and stored
  `offset === stats.size`, the file is skipped without opening.
- **Append vs rewrite:** Grown files resume at `offset`. Shrunk files restart from byte 0.
- **Offset semantics:** `offset` covers bytes up to the last line ending in `\n`. Torn final lines
  remain outside the consumed byte range and are re-read on the next sync pass.
...
- **Account grouping:** Groups limits by `(provider, account_key)` and surfaces `email`.
...
- `ompEnabled`, `cursorEnabled`, `claudeEnabled`: boolean toggles; all default to `true`.
...
`claude_path`, `omp_enabled`, `claude_enabled`, `cursor_enabled`) are loaded by `packages/reader`
and forwarded here.
...
- **Node built-ins:** `node:fs`, `node:path`, `node:os`, `node:crypto`, `node:child_process`
  ...
  `ollama.ts`, `omp-limits.ts`, and `sync.ts`).
...
- `cursor.test.ts`: cycle snapshot and calendar window aggregation from captured fixtures, omission
  of unreturned cache counts, origin/cookie header enforcement, and HTTP error handling.
...
- `collect.test.ts`: parallel orchestrator execution across all sources, source isolation on error,
  credential exclusion from DB, disabled source handling (including `claudeEnabled: false`), and
  independent Antigravity execution when OMP is disabled.
...
- **Whole file read into memory:** Both transcript scans perform `readFileSync` over whole files,
  which is manageable for ordinary session logs but expensive for pathological files.
- **Silent error swallowing:** Unparsable lines, vanished files, and unreadable directories are
  swallowed by design. Debugging missing data requires inspecting directories directly.
...
- **Adding a new transcript source:**
  ...
  3. Create the transcript parser and scanner module implementing `ScanFile`.
  4. Add an entry point in `src/sync.ts` delegating to `syncTranscripts`.
